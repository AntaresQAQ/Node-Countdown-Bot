...
function addSaying() {
    let saying = randomSelect(Sayings)
    saying = saying.replace("曾经说过", randomSelect(FrontWords))
    saying = saying.replace("这不禁令我深思", randomSelect(BackWords))
    return saying
}

function addDiscourse(subject) {
    return randomSelect(Discourses).replace(/主题/g, subject);
}
...
function generate(subject, length) {
    let article = [];
    let chapter = "";
    let chapter_length = 0;
    while (chapter_length < length) {
        let random_number = random(0, 100);
        if ((random_number < 5 && chapter.length > 20) || chapter.length > 200) {
            chapter = addParagraph(chapter);
            article.push(chapter);
            chapter = "";
        } else if (random_number < 20) {
            let sentence = addSaying();
            chapter_length = chapter_length + sentence.length;
            chapter += sentence;
        } else {
            let sentence = addDiscourse(subject);
            chapter_length += sentence.length;
            chapter += sentence;
        }
    }
    chapter = addParagraph(chapter);
